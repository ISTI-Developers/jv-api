...
try {
    ...
    $db->exec("UPDATE master_revenue_account_titles SET is_enabled = 0");

    $upsert = $db->prepare("
        INSERT INTO master_revenue_account_titles (
            account_no,
            account_title,
            is_enabled,
            created_at,
    ...
    try {
        logAudit(
            $db,
            (int) $admin['id'],
            'UPDATE_MASTER_REVENUE_ACCOUNT_TITLES',
            'MASTER_LIST',
            'master_revenue_account_titles',
            null,
            [
            ...
            'Admin updated master revenue account title allowlist'
        );
    } catch (Throwable $e) {
    ...
    echo json_encode([
        'success' => true,
        'message' => 'Revenue account titles updated successfully',
        'enabled_count' => count($enabledAccountTitles),
    ]);
} catch (Throwable $e) {
